...
const fullName = 'Demo Parent';
const role = 'Parent';
const studentId = 'NKQMS-2026-8775'; // GODGIFT GOODNEWS
...
async function main() {
    ...
    if (!signupRes.ok) {
        ...
        // Try to insert profile if it doesn't exist (using user's bearer token)
    ...
    // 2. Link parent to student in parent_links table
    console.log(`[Link] Upserting parent link from ${userId} to student ${studentId}...`);
    ...
    if (!linkRes.ok) {
        console.error('Error creating parent link:', await linkRes.json());
    } else {
        console.log('✅ Parent link established successfully!');
    }
}
