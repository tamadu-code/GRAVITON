...
export const UI = {
        ...
        overlay.innerHTML = `
            <div class="modal-container">
                ...
                <div class="modal-footer">
                    <button class="btn btn-secondary" onclick="document.getElementById('ui-modal').remove()" style="background: #f1f5f9; color: #475569; border: 1px solid #e2e8f0; padding: 0.75rem 1.5rem; border-radius: 12px; font-weight: 600;">Cancel</button>
    ...
    async renderStudentDashboard() {
                        ...
                        <div class="kpi-visualization-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 1.5rem;">
                            <div style="background: white; border-radius: 24px; padding: 1.5rem; border: 1px solid #e2e8f0; display: flex; flex-direction: column; gap: 1rem; transition: all 0.3s ease; cursor: pointer; position: relative;" onmouseover="this.style.borderColor='#4f46e5'; this.style.transform='translateY(-5px)'" onmouseout="this.style.borderColor='#e2e8f0'; this.style.transform='none'">
                                ...
                                <div>
                                    ...
                                    <div style="font-size: 1.75rem; font-weight: 900; color: #1e293b; margin-top: 2px;">${displayAvg}</div>
                                </div>
                            ...
                            <div style="background: white; border-radius: 24px; padding: 1.5rem; border: 1px solid #e2e8f0; display: flex; flex-direction: column; gap: 1rem; transition: all 0.3s ease; cursor: pointer; position: relative;" onmouseover="this.style.borderColor='#8b5cf6'; this.style.transform='translateY(-5px)'" onmouseout="this.style.borderColor='#e2e8f0'; this.style.transform='none'">
                                ...
                                <div>
                                    <div style="font-size: 0.7rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em;">Current Rank</div>
                                    <div style="font-size: 1.75rem; font-weight: 900; color: #1e293b; margin-top: 2px;">${displayRank}</div>
                                </div>
                                ${hasFeeBalance ? `<div style="position: absolute; inset: 0; background: rgba(255,255,255,0.8); border-radius: 24px; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(4px);"><i data-lucide="lock" style="color: #ef4444; width: 24px;"></i></div>` : ''}
                            </div>
                            ...
                            <div style="background: white; border-radius: 24px; padding: 1.5rem; border: 1px solid #e2e8f0; display: flex; flex-direction: column; gap: 1rem; transition: all 0.3s ease; cursor: pointer;" onmouseover="this.style.borderColor='#10b981'; this.style.transform='translateY(-5px)'" onmouseout="this.style.borderColor='#e2e8f0'; this.style.transform='none'">
                                ...
                                <div>
                                    ...
                                    <div style="font-size: 1.75rem; font-weight: 900; color: #1e293b; margin-top: 2px;">${analytics.attendance_rate || 0}%</div>
                                </div>
                            </div>
                        </div>
        ...
        this.renderTimetableForStudent(studentId);
        this.renderPaymentHistory(studentId);
    },

    async renderTimetableForStudent(studentId) {
        const student = await db.students.get(studentId);
        if (student && student.class_name) {
            await this.renderTimetableVisualizer(student.class_name);
        }
    },

    printReceipt(paymentId) {
        Notifications.show('Generating digital receipt...', 'info');
        // Simple print-friendly view for now
        window.open(`receipt.html?id=${paymentId}`, '_blank');
    },
    ...
    async modifySubject(subjectName, currentIds) {
        ...
        const modalHtml = `
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                <div>
                    <label style="font-weight:700; color:var(--text-primary);">Course Title</label>
                    <input type="text" id="edit-sub-name" class="input" value="${subjectName}" style="width:100%; height: 44px; border-radius: 10px; background: #f8fafc; color: #1e293b; border: 1px solid #e2e8f0;">
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div>
                        <label style="font-weight:700; color:var(--text-primary);">Module Type</label>
                    ...
                    <div>
                        <label style="font-weight:700; color:var(--text-primary);">Credit Load</label>
                        <input type="number" id="edit-sub-credits" class="input" value="${subject.credits || 1}" style="width:100%; height: 44px; border-radius: 10px; background: #f8fafc; color: #1e293b; border: 1px solid #e2e8f0;">
                    </div>
                </div>

                <div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <label style="font-weight:700; color:var(--text-primary);">Stream Assignments</label>
                        <button type="button" id="btn-add-assign" style="background: #eff6ff; color: #2563eb; border: 1px solid #bfdbfe; border-radius: 8px; padding: 0.3rem 0.6rem; font-size: 0.7rem; font-weight: 700; cursor: pointer;">+ Add Assignment</button>
                    </div>
                    <div id="assignment-list" style="display: flex; flex-direction: column; gap: 0.5rem; max-height: 200px; overflow-y: auto; background: #f8fafc; padding: 0.5rem; border-radius: 8px;">
                        ${currentAssignments.length === 0 ? '<p style="font-size:0.7rem; color:#94a3b8; text-align:center; padding:1rem;">No assignments. Click + Add Assignment.</p>' : currentAssignments.map(a => `
                            <div class="assignment-row" style="display: flex; gap: 0.4rem; align-items: center; background: white; padding: 0.4rem; border-radius: 6px; border: 1px solid #e2e8f0;">
                                <select class="input assign-teacher" style="flex: 1.5; height: 36px; font-size: 0.8rem; border:none; background:transparent;">
                                    <option value="">No Teacher</option>
                                    ${teachers.map(t => `<option value="${t.id}" ${a.teacher_id === t.id ? 'selected' : ''}>${t.full_name || t.username}</option>`).join('')}
                                </select>
                                <select class="input assign-class" style="flex: 1.2; height: 36px; font-size: 0.8rem; border:none; background:transparent;">
                                    ${classes.map(c => `<option value="${c.name}" ${a.class_name === c.name ? 'selected' : ''}>${c.name}</option>`).join('')}
                                </select>
                                <select class="input assign-spec" style="flex: 1; height: 36px; font-size: 0.8rem; border:none; background:transparent;">
                                ...
                                <button type="button" onclick="this.parentElement.remove()" style="color:#ef4444; background:none; border:none; cursor:pointer;"><i data-lucide="trash-2" style="width:14px;"></i></button>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        document.getElementById('btn-add-assign').onclick = () => {
            ...
            row.style = "display: flex; gap: 0.4rem; align-items: center; background: white; padding: 0.4rem; border-radius: 6px; border: 1px solid #e2e8f0;";
            row.innerHTML = `
                <select class="input assign-teacher" style="flex: 1.5; height: 36px; font-size: 0.8rem; border:none; background:transparent;">
                    <option value="">No Teacher</option>
    ...
    async renderGrades() {
        ...
        classFilter.addEventListener('change', async (e) => {
            ...
            if (availableSubjects.length > 0) {
                ...
                // Fallback: If no assignments but user is admin, maybe show all subjects or at least notify
                        ...
                        <div class="glass-collapse-content" style="border-top: 1px solid #f1f5f9; background: #f8fafc; border-radius: 0 0 16px 16px;">
                            <div style="padding: 1rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0.75rem;">
                                ...
                                <div style="grid-column: 1 / -1; margin-top: 0.25rem;">
                                    <label style="display: block; font-size: 0.6rem; font-weight: 800; color: #94a3b8; text-transform: uppercase; margin-bottom: 0.5rem;">${currentTab === 'school' ? 'Sync Status' : 'Manual Override'}</label>
                                    ${currentTab === 'school' ? `
                                        <div style="background: #f1f5f9; color: #64748b; font-size: 0.7rem; font-weight: 700; padding: 0.6rem; border-radius: 8px; text-align: center; border: 1px solid #e2e8f0; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                            ...
                            <div class="cbt-exam-card" style="background: white; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; transition: all 0.3s ease; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                                ...
                                <div class="cbt-exam-content" style="max-height: 0; overflow: hidden; transition: max-height 0.4s ease-out; background: #f8fafc; border-top: 1px solid #f1f5f9;">
                                    <div style="padding: 1.5rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1.5rem;">
                                        ${(result && result.status === 'Completed' && isStudent) ? `
                                            <div style="grid-column: 1 / -1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 2rem 1rem; background: #ecfdf5; border-radius: 16px; border: 1px solid #d1fae5; text-align: center;">
                                                <div style="width: 48px; height: 48px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #10b981; margin-bottom: 1rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                                                    <i data-lucide="award" style="width: 24px; height: 24px;"></i>
                                                </div>
                                                <div style="font-size: 0.75rem; font-weight: 800; color: #059669; margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.1em;">Final Assessment Score</div>
                                                <div style="font-weight: 900; color: #064e3b; font-size: 3rem; line-height: 1; margin-bottom: 0.5rem;">
                                                    ${result.score.toFixed(1)} <span style="font-size: 1.25rem; color: #059669; font-weight: 700; opacity: 0.8;">/ ${(result.total_marks || result.total_questions).toFixed(1)}</span>
                                                </div>
    ...
    async renderCBTInstructions(examId) {
        ...
        this.contentArea.innerHTML = `
            <div class="view-container animate-fade-in" style="max-width: 800px; margin: 0 auto; padding: clamp(0.5rem, 3vw, 2rem);">
                <div class="card" style="background: white; border-radius: 24px; padding: clamp(1.25rem, 5vw, 3rem); border: none; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
                    <div class="text-center" style="margin-bottom: 2rem;">
                        <div style="background: #e0e7ff; color: #4338ca; width: 64px; height: 64px; border-radius: 20px; display: flex; align-items: center; justify-content: center; margin: 0 auto 1rem; transform: rotate(-5deg);">
                            <i data-lucide="info" style="width: 32px; height: 32px;"></i>
                        </div>
                        <h1 style="font-weight: 900; font-size: clamp(1.2rem, 5vw, 1.75rem); color: #1e293b; margin-bottom: 0.5rem;">Examination Instructions</h1>
                        <p style="color: #64748b; font-size: clamp(0.85rem, 3vw, 1rem);">Please read carefully before starting your attempt.</p>
                    </div>
